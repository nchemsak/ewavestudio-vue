<template>
    <button class="wave-btn" type="button" role="switch" :class="[{ 'is-active': modelValue, 'is-disabled': disabled }]"
        ...
        <ReverbIcon class="wave-svg" :enabled="modelValue" :palette="palette" />
        <span class="wave-label">{{ label }}</span>
    </button>
</template>

<script setup lang="ts">
import ReverbIcon from './ReverbIcon.vue';
...
withDefaults(defineProps<{
    ...
    label: 'Reverb',
    disabled: false,
    palette: () => ['#7c9bff', '#9fe0ff', '#cba6ff']  // cool spacious gradient
});
...
<style scoped>
/* Same tile style family as Unison/Delay/Drive */
.wave-btn {
